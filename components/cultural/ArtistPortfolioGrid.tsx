...
const ArtistPortfolioGrid: React.FC<ArtistPortfolioGridProps> = ({
  ...
  const filteredArtists = useMemo(() => {
    return artists.filter(artist => {
      ...
      const matchesFilters = Object.entries(filters).every(([key, value]) => {
        ...
        switch (key) {
          case 'school':
            return artist.school === value
          case 'period':
            return artist.period === value
          case 'specialty':
            return artist.specialty.includes(value)
          case 'status':
            return artist.status === value
          default:
            return true
        }
      })
  ...
  const ArtistCard: React.FC<{ artist: Artist; index: number }> = ({ artist, index }) => (
    ...
    >
      ...
      <CardContent
        className={cn('p-4 flex-1', currentLayout === 'list' ? 'flex flex-col justify-center' : '')}
      >
        {/* Artist Name */}
        <div className='mb-3'>
          ...
          <p className='font-english text-sm text-ink-black/70'>{artist.name.romanized}</p>
        </div>
    ...
    <div className={cn('space-y-6', className)}>
      ...
      >
        {paginatedArtists.map((artist, index) => (
          <div key={artist.id} className={currentLayout === 'masonry' ? 'break-inside-avoid' : ''}>
