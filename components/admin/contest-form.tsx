  ...
  const onSubmit = async (data: ContestFormData) => {
    try {
      ...
      if (contest) {
        ...
        if (updateError)
          throw new Error(typeof updateError === 'string' ? updateError : '저장에 실패했습니다')
      } else {
        // Create new contest
        const { error: createError } = await createContest(data, clerkUser?.id ?? '')
        if (createError)
          throw new Error(typeof createError === 'string' ? createError : '생성에 실패했습니다')
      }
        ...
        <CardContent className='space-y-4'>
          ...
          <div>
            ...
            {form.formState.errors.description && (
              <p className='text-sm text-scholar-red mt-1'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div>
              ...
              <Select
                ...
                onValueChange={value => form.setValue('category', value as ContestCategory)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                value={form.watch('contestType')}
                onValueChange={value => form.setValue('contestType', value as ContestType)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
        ...
        <CardContent className='space-y-4'>
          <div>
            ...
            <Select
              ...
              onValueChange={value => form.setValue('status', value as ContestStatus)}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
