...
function MetricCard({ name, value, score, description, icon }: MetricCardProps) {
  ...
  const ScoreIcon =
    score === 'good' ? CheckCircle : score === 'needs-improvement' ? AlertTriangle : XCircle
...
function PerformanceHistory({ metrics }: PerformanceHistoryProps) {
  const groupedMetrics = (metrics as any[]).reduce(
    (acc, metric) => {
          ...
          <Card key={metricName}>
            ...
            <CardContent>
              <div className='flex items-center justify-between'>
                ...
                <Badge
                  variant={
                    latest.score === 'good'
                      ? 'default'
                      : latest.score === 'needs-improvement'
                        ? 'secondary'
                        : 'destructive'
                  }
                >
              ...
              <div className='text-xs text-muted-foreground mt-1'>
                {(metricList as any[]).length} measurement
                {(metricList as any[]).length > 1 ? 's' : ''}
              </div>
            </CardContent>
          </Card>
        )
...
function PerformanceInsights({ monitor }: PerformanceInsightsProps) {
  ...
  insights.push({
    type:
      performanceScore.overall === 'good'
        ? 'success'
        : performanceScore.overall === 'needs-improvement'
          ? 'warning'
          : 'error',
    title: '전체 성능 점수',
...
export default function PerformanceDashboard() {
  ...
  const latestMetrics = Object.keys(PERFORMANCE_THRESHOLDS)
    .map(metricName => {
        ...
        <CardContent>
          <div className='flex items-center space-x-4'>
            <div
              ...
              style={{ color: getMetricColor(performanceScore.overall) }}
            >
              {performanceScore.overall === 'good'
                ? '좋음'
                : performanceScore.overall === 'needs-improvement'
                ...
                <div>
                  ...
                  <div className='space-y-2 text-sm'>
                    <div className='flex justify-between'>
                      <span>페이지 로딩 시간:</span>
                      <span>
                        {analytics.pageLoadTime > 0
                          ? `${(analytics.pageLoadTime / 1000).toFixed(2)}초`
                          : 'N/A'}
                      </span>
                    </div>
                    <div className='flex justify-between'>
                      <span>렌더링 시간:</span>
                      <span>
                        {analytics.renderTime > 0 ? `${analytics.renderTime.toFixed(0)}ms` : 'N/A'}
                      </span>
                    </div>
                    <div className='flex justify-between'>
                      <span>첫 상호작용 시간:</span>
                      <span>
                        {analytics.interactionTime > 0
                          ? `${analytics.interactionTime.toFixed(0)}ms`
                          : 'N/A'}
                      </span>
                    </div>
                  </div>
