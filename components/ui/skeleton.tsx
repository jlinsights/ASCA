    ...
    <div className={cn('space-y-2', className)}>
      {Array.from({ length: lines }, (_, i) => (
        <Skeleton
          key={i}
          variant={variant}
          className={cn('h-4', i === lines - 1 && lastLineWidth !== '100%' && 'w-3/5')}
...
function SkeletonCard({
  ...
  return (
    <div className={cn('space-y-4 p-4 border rounded-lg', className)}>
      {showImage && <Skeleton variant={variant} className='h-48 w-full' />}

      <div className='space-y-2'>
        {showTitle && <Skeleton variant={variant} className='h-6 w-3/4' />}
    ...
    <div className={cn('space-y-3', className)}>
      {Array.from({ length: items }, (_, i) => (
        <div key={i} className='flex items-center space-x-3'>
          {showAvatar && <Skeleton variant={variant} className='h-10 w-10 rounded-full' />}
          {showIcon && <Skeleton variant={variant} className='h-6 w-6' />}
...
export { Skeleton, SkeletonText, SkeletonCard, SkeletonList, LoadingState, useLoadingState }
