        ...
        <div className='space-y-6'>
          ...
          <div className='relative'>
            ...
            <div className='flex items-end gap-4 px-6 -mt-8 relative'>
                ...
                <div className='flex items-center gap-4 mt-2 text-sm text-muted-foreground'>
                  ...
                  {contactInfo.website && (
                    <a
                      ...
                      className='flex items-center gap-1 hover:text-celadon'
                    >
              ...
              <Button onClick={() => onToggleFollow(artist.id)} className='mb-2'>
                <Users
                  className={`h-4 w-4 mr-2 ${followedArtists.has(artist.id) ? 'fill-current' : ''}`}
                />
                        ...
                        <div className='flex items-start justify-between'>
                          <div>
                            <h4 className='font-medium mb-1'>
                              {typeof exhibition === 'string'
                                ? exhibition
                                : (exhibition as any).title || '전시명'}
                            </h4>
                            <p className='text-sm text-muted-foreground mb-1'>
                              {typeof exhibition === 'string'
                                ? '장소 미상'
                                : (exhibition as any).venue || '장소 미상'}
                            </p>
                            <div className='flex items-center gap-2'>
                              <Badge variant='outline' className='text-xs'>
                                {typeof exhibition === 'string'
                                  ? '전시'
                                  : (exhibition as any).type || '전시'}
                              </Badge>
                              <span className='text-xs text-muted-foreground'>
                                {typeof exhibition === 'string'
                                  ? '연도 미상'
                                  : (exhibition as any).year || '연도 미상'}
                              </span>
                            </div>
                          </div>
