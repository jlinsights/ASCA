...
const DropdownMenu = memo(
  ({ subItems, isActive }: { subItems: MenuItem['subItems']; isActive: boolean }) => (
    <div
      className={`
...
const NavigationItem = memo(
  ({
    ...
    >
      ...
      {menu.subItems.length > 0 && <DropdownMenu subItems={menu.subItems} isActive={isActive} />}
    </div>
  )
)
...
export const HeaderNavigation = memo(
  ({ menuStructure, activeDropdown, onMouseEnter, onMouseLeave }: HeaderNavigationProps) => (
