        ...
        <CardContent>
          <div className='space-y-4'>
            {exhibitions.map((exhibition: any) => {
              const isActive =
                new Date(exhibition.schedule.opening_date) <= new Date() &&
                (!exhibition.schedule.closing_date ||
                  new Date(exhibition.schedule.closing_date) >= new Date())
                        ...
                        <div className='grid grid-cols-2 gap-4 text-sm'>
                          <div>
                            ...
                            <span className='ml-2 font-medium'>
                              {exhibition.curation.chief_curator}
                            </span>
                          </div>
                          <div>
                            <span className='text-ink-black/70'>Sections:</span>
                            <span className='ml-2 font-medium'>
                              {exhibition.layout.sections.length}
                            </span>
                          </div>
                          <div>
