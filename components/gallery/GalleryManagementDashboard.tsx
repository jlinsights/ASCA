...
const CollectionsTab = dynamic(() =>
  import('./dashboard/CollectionsTab').then(mod => mod.CollectionsTab)
)
const ExhibitionsTab = dynamic(() =>
  import('./dashboard/ExhibitionsTab').then(mod => mod.ExhibitionsTab)
)
...
const GalleryManagementDashboard: React.FC<GalleryManagementDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'artworks' | 'collections' | 'exhibitions' | 'analytics'
  >('overview')
  ...
  const filteredArtworks = React.useMemo(() => {
    return artworks.filter(artwork => {
      ...
      if (filters.visibility !== 'all' && artwork.metadata.visibility !== filters.visibility)
        return false
      if (
        filters.artist &&
        !artwork.artist.name.toLowerCase().includes(filters.artist.toLowerCase())
      )
        return false
      if (filters.search) {
    ...
    try {
      ...
      if (action) {
        ...
        logger.info('Bulk action completed', {
          actionId,
          itemCount: Array.from(selectedItems).length,
        })
        setSelectedItems(new Set())
  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      <ErrorBoundary
        fallback={
          <div className='p-4 m-4 bg-red-50 text-red-800 rounded-md'>
            오류: 탭 컴포넌트를 불러올 수 없습니다.
          </div>
        }
      >
        {activeTab === 'overview' && (
