...
import { Eye, Heart, Edit, Trash2, MoreVertical, ExternalLink, Star } from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  return (
    <Card className='group overflow-hidden border border-celadon-green/20 hover:border-celadon-green/40 transition-all duration-300 hover:shadow-lg bg-rice-paper dark:bg-card'>
      <CardHeader className='p-0 relative'>
        ...
        <div className='relative aspect-[4/3] overflow-hidden bg-stone-gray/10'>
          ...
          <div className='absolute top-3 left-3 flex gap-2'>
            <Badge className={`${statusInfo.color} text-white`}>{statusInfo.ko}</Badge>
            {artwork.isFeatured && (
      ...
      <CardContent className='p-4'>
        ...
        <Badge
          variant='outline'
          className='mb-2 text-xs border-celadon-green/30 text-celadon-green'
        >
          {categoryLabel}
        </Badge>
        ...
        {/* 영문 제목 (있는 경우) */}
        {artwork.titleEn && (
          <p className='text-sm text-muted-foreground mb-2 line-clamp-1'>{artwork.titleEn}</p>
        )}
