...
async function safeDuplicateRemoval() {
  ...
  try {
    ...
    while (true) {
      ...
      console.log(`   처리 중: ${processedCount}/${totalCount} (${Math.round(processedCount/totalCount*100)}%)`);
    ...
    for (const { name, artists } of allDuplicates.slice(0, 100)) { // 안전을 위해 처음 100개만
      try {
