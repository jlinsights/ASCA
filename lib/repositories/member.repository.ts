...
export class MemberRepository extends BaseRepository<typeof members, Member, NewMember> {
  ...
  async search(
    ...
    const orderBy = criteria.sortOrder === 'asc' ? asc(sortColumn) : desc(sortColumn)
   ...
   */
  async membershipNumberExists(membershipNumber: string, excludeId?: string): Promise<boolean> {
    const where = excludeId
      ? and(eq(members.membershipNumber, membershipNumber), sql`${members.id} != ${excludeId}`)
  ...
  async bulkUpdate(ids: string[], data: Partial<NewMember>): Promise<Member[]> {
    return this.updateMany(sql`${members.id} = ANY(${ids})`, {
      ...data,
      updatedAt: new Date(),
    })
  }
