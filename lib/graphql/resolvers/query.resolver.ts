...
export const queryResolvers = {
  ...
  upcomingEvents: async (_: any, { limit = 10 }: { limit?: number }, context: GraphQLContext) => {
    const now = new Date()

    const events = await context.db.query.events.findMany({
      where: and(eq(schema.events.status, 'upcoming'), sql`${schema.events.startDate} > ${now}`),
  ...
  featuredNews: async (_: any, { limit = 5 }: { limit?: number }, context: GraphQLContext) => {
    const newsItems = await context.db.query.news.findMany({
      where: and(eq(schema.news.isPinned, true), eq(schema.news.status, 'published')),
      limit,
      orderBy: [desc(schema.news.publishedAt)],
    })
