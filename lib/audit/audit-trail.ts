...
export class AuditTrail {
  ...
  generateComplianceReport(timeRange: number = 30 * 24 * 60 * 60 * 1000): {
    ...
    const successRate =
      relevantEntries.length > 0 ? successfulOperations / relevantEntries.length : 0

    const totalRiskScore = relevantEntries.reduce((sum, e) => sum + e.riskScore, 0)
    const averageRiskScore =
      relevantEntries.length > 0 ? totalRiskScore / relevantEntries.length : 0

    const criticalViolations = relevantEntries.filter(
      e => e.riskScore > 0.8 || e.result === 'failure'
    )
