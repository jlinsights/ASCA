...
export const memberLastActivityIndex = index('member_last_activity_idx').on(
  members.lastActivityDate
)

// Member Activity 인덱스
export const memberActivityMemberIdIndex = index('member_activity_member_id_idx').on(
  memberActivities.memberId
)
export const memberActivityTypeIndex = index('member_activity_type_idx').on(
  memberActivities.activityType
)
export const memberActivityTimestampIndex = index('member_activity_timestamp_idx').on(
  memberActivities.timestamp
)
export const memberActivityEntityIndex = index('member_activity_entity_idx').on(
  memberActivities.relatedEntityType,
  memberActivities.relatedEntityId
)

// Cultural Exchange Program 인덱스
export const culturalProgramTypeIndex = index('cultural_program_type_idx').on(
  culturalExchangePrograms.programType
)
export const culturalProgramStatusIndex = index('cultural_program_status_idx').on(
  culturalExchangePrograms.status
)
export const culturalProgramStartDateIndex = index('cultural_program_start_date_idx').on(
  culturalExchangePrograms.startDate
)
export const culturalProgramDeadlineIndex = index('cultural_program_deadline_idx').on(
  culturalExchangePrograms.applicationDeadline
)
export const culturalProgramFeaturedIndex = index('cultural_program_featured_idx').on(
  culturalExchangePrograms.isFeatured
)

// Cultural Exchange Participant 인덱스
export const culturalParticipantProgramIdIndex = index('cultural_participant_program_id_idx').on(
  culturalExchangeParticipants.programId
)
export const culturalParticipantMemberIdIndex = index('cultural_participant_member_id_idx').on(
  culturalExchangeParticipants.memberId
)
export const culturalParticipantStatusIndex = index('cultural_participant_status_idx').on(
  culturalExchangeParticipants.status
)
export const culturalParticipantPaymentIndex = index('cultural_participant_payment_idx').on(
  culturalExchangeParticipants.paymentStatus
)
export const culturalParticipantAppliedAtIndex = index('cultural_participant_applied_at_idx').on(
  culturalExchangeParticipants.appliedAt
)

// Calligraphy Analysis 인덱스
export const analysisUserIdIndex = index('analysis_user_id_idx').on(calligraphyAnalyses.userId)
export const analysisMemberIdIndex = index('analysis_member_id_idx').on(
  calligraphyAnalyses.memberId
)
export const analysisStyleIndex = index('analysis_style_idx').on(
  calligraphyAnalyses.calligraphyStyle
)
...
export const analysisCreatedAtIndex = index('analysis_created_at_idx').on(
  calligraphyAnalyses.createdAt
)

// Learning Progress 인덱스
export const learningProgressMemberIdIndex = index('learning_progress_member_id_idx').on(
  learningProgress.memberId
)
export const learningProgressCategoryIndex = index('learning_progress_category_idx').on(
  learningProgress.skillCategory
)
export const learningProgressLevelIndex = index('learning_progress_level_idx').on(
  learningProgress.currentLevel
)
export const learningProgressLastPracticeIndex = index('learning_progress_last_practice_idx').on(
  learningProgress.lastPracticeDate
)
...
export const fileStorageEntityIndex = index('file_storage_entity_idx').on(
  fileStorage.relatedEntityType,
  fileStorage.relatedEntityId
)
export const fileStorageStatusIndex = index('file_storage_status_idx').on(
  fileStorage.processingStatus
)
...
export const notificationPriorityIndex = index('notification_priority_idx').on(
  notifications.priority
)
export const notificationCreatedAtIndex = index('notification_created_at_idx').on(
  notifications.createdAt
)

// Performance Metrics 인덱스
export const performanceMetricTypeIndex = index('performance_metric_type_idx').on(
  performanceMetrics.metricType
)
export const performanceMetricEndpointIndex = index('performance_metric_endpoint_idx').on(
  performanceMetrics.endpoint
)
export const performanceMetricTimestampIndex = index('performance_metric_timestamp_idx').on(
  performanceMetrics.timestamp
)
export const performanceMetricUserIdIndex = index('performance_metric_user_id_idx').on(
  performanceMetrics.userId
)

// 복합 인덱스
export const artworkArtistCategoryIndex = index('artwork_artist_category_idx').on(
  artworks.artistId,
  artworks.category
)
export const memberTierStatusIndex = index('member_tier_status_idx').on(
  members.tierLevel,
  members.status
)
export const analysisUserStyleIndex = index('analysis_user_style_idx').on(
  calligraphyAnalyses.userId,
  calligraphyAnalyses.calligraphyStyle
)
export const activityMemberTypeIndex = index('activity_member_type_idx').on(
  memberActivities.memberId,
  memberActivities.activityType
)
export const notificationUserReadIndex = index('notification_user_read_idx').on(
