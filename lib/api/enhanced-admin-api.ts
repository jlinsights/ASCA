...
export class CreateArtistHandler implements CommandHandler {
  async handle(command: Command): Promise<CommandResult<Artist>> {
    ...
    try {
      ...
      // 감사 로그 기록
      await auditTrail.trackDataChange(userId, 'artist.create', 'artist', data.id, null, data)
  ...
  async handle(query: Query): Promise<QueryResult<Artist[]>> {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .or(
          `name.ilike.%${searchQuery}%,bio.ilike.%${searchQuery}%,nationality.ilike.%${searchQuery}%`
        )
        .order('created_at', { ascending: false })
