...
export async function fetchCareerEntries(
  ...
  let query = supabase.from('career_timeline').select('*').eq('artist_id', artistId)
  ...
  if (filters?.yearRange) {
    query = query.gte('year', filters.yearRange.start).lte('year', filters.yearRange.end)
  }
...
export async function fetchCareerEntryById(
  ...
  const { data, error } = await supabase.from('career_timeline').select('*').eq('id', id).single()
 ...
 */
export async function toggleCareerFeatured(
  id: string
): Promise<{ data: CareerEntry | null; error: any }> {
  const supabase = getSupabaseClient()
...
export async function updateCareerDisplayOrder(
  ...
  const promises = updates.map(({ id, order }) =>
    supabase.from('career_timeline').update({ display_order: order }).eq('id', id)
  )
