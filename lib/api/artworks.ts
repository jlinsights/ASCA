...
export async function fetchArtworks(
  ...
  if (filters) {
    ...
    if (filters.searchTerm) {
      query = query.or(
        `title.ilike.%${filters.searchTerm}%,description.ilike.%${filters.searchTerm}%,medium.ilike.%${filters.searchTerm}%`
      )
    }
  }
...
export async function fetchArtworksByArtist(
  ...
  let query = supabase.from('artworks').select('*').eq('artist_id', artistId)
 ...
 */
export async function toggleFeatured(id: string): Promise<{ data: Artwork | null; error: any }> {
  const supabase = getSupabaseClient()
  if (!supabase) return { data: null, error: { message: 'Supabase not configured' } }
...
export async function getArtworkStats(
  artistId: string
): Promise<{ data: ArtworkStats | null; error: any }> {
  const supabase = getSupabaseClient()
  if (!supabase) return { data: null, error: { message: 'Supabase not configured' } }
  ...
  artworks.forEach(artwork => {
    ...
    stats.artworksByCategory[category] = (stats.artworksByCategory[category] || 0) + 1
  })
  ...
  if (forSaleArtworks.length > 0) {
    stats.averagePrice =
      forSaleArtworks.reduce((sum, a) => sum + (a.price || 0), 0) / forSaleArtworks.length
  }
