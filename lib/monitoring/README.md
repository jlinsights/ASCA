...
// 동기 함수 측정
const processData = measureSync('data.process', (data: any) => {
...
// 여러 윈도우에 대한 집계
const multiWindow =
  metricsCollector.aggregateMultipleWindows('api.response_time')
```
...
try {
  ...
  slowQueryDetector.endQuery(queryId, 'SELECT * FROM users WHERE id = ?', [
    userId,
  ])
  throw error
}
