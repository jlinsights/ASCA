...
export class MetricsCollector {
   ...
   * 집계된 메트릭 조회
   */
  getAggregatedMetrics(metricName?: string, window?: TimeWindow): AggregatedMetric[] {
   ...
   * 이상 데이터 조회
   */
  getAnomalies(metricName?: string, severity?: 'low' | 'medium' | 'high'): AnomalyDetection[] {
  ...
  getStats(): {
    ...
    for (const [key, metrics] of this.aggregatedMetrics.entries()) {
      ...
      metricsPerWindow[window || 'unknown'] =
        (metricsPerWindow[window || 'unknown'] || 0) + metrics.length
    }
