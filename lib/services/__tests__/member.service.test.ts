...
describe('MemberService', () => {
  ...
  describe('updateMember', () => {
    ...
    test('should throw conflict error for duplicate email', async () => {
      ...
      // Act & Assert
      await expect(
        memberService.updateMember(mockMember.id, updateData)
  ...
  describe('updateMemberLevel', () => {
    ...
    test('should throw not found for non-existent member', async () => {
      ...
      // Act & Assert
      await expect(
        memberService.updateMemberLevel('invalid-id', 'level-id')
      ).rejects.toThrow('Member not found');
    });
  });
  ...
  describe('bulkApproveMember', () => {
    test('should approve multiple members', async () => {
      ...
      mockRepository.updateStatus = jest.fn()
    ...
    test('should continue on error when one approval fails', async () => {
      ...
      mockRepository.updateStatus = jest.fn()
        .mockResolvedValueOnce({ ...mockPendingMember, id: 'id1', membership_status: 'active' as const })
        .mockResolvedValueOnce({ ...mockPendingMember, id: 'id3', membership_status: 'active' as const });
