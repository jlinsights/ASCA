...
⏱️ Realtime 4 file `jest.useFakeTimers + setTimeout` 충돌 일괄 fix.
canonical 패턴 `jest.advanceTimersByTimeAsync(N)` 로 40 occurrences replace_all.
PR #18 admin merge `598ec889`. CI passed 402→419 (**+17**, mini-do +16 예측 +1 차이).
...
- `await new Promise(resolve => setTimeout(resolve, N))` → resolve 안 됨 → 5000ms timeout
