...
- 부모 사이클 `tests-route-mutation-auth` (PR #16) 잔여 F 그룹 (Error Handling 3
  fail)
...
- **추가 발견**: vm-realm 차이로 `instanceof SyntaxError` 실패 →
  `.name === 'SyntaxError'` 비교
...
- Apollo Server v4: validation error = 400, parse 실패 = throw, resolver throw =
  200+errors
