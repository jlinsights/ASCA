...
정확). **13 chain 누적 228→432 (+204, +89%)**. rev β 10연속 검증 (Match avg
98.5%).
...
**Problem**: thenable mock의 `Promise.resolve(this)` — this가 thenable이라 무한
재귀
...
4. **Spike → Fix scope 분리** (Karpathy §2 simplicity) — 본 사이클이 spike
   후속의 좋은 예
