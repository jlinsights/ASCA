...
2026-03-28 코드 분석 결과, ASCA 프로젝트에서 **Critical 보안 이슈 7건**이
발견됨. SQL Injection, XSS, 인증 미구현, 민감 정보 노출 등 배포 차단 수준의
취약점이 포함.
...
- **현상**: `executeSchemaChange`에서 `tableName`, `columnName`을 직접 SQL
  문자열에 삽입
...
- **현상**: 외부 RSS 콘텐츠를 `dangerouslySetInnerHTML`로 렌더링, `<script>` 등
  미제거
...
**작성일**: 2026-03-28 **PDCA Phase**: Plan **다음 단계**:
`/pdca design security-hardening`
