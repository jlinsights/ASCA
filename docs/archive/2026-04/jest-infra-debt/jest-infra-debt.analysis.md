...
>
> **권고**: 외부 차단(CI/CD)이 남아 있어 `iterate` 불필요. **Split Cycle** 원칙
> 적용 — Phase 6(CI 통합)은 별 세션 또는 단순 머지 워크플로로 분리.
...
- Design §3.3에서 가설 H1(F1+F4 후 자동 해결) 우선 가정했으나, 실제로는 H1 false
  positive
- **실제 root cause 발견**: `import { jest } from '@jest/globals'`이
  babel-jest의 `jest.mock()` hoisting 비활성화
- **적용된 fix**: `@jest/globals`에서 `jest` import 제거 → 글로벌 `jest` 사용 →
  hoisting 복원
...
- 변수 이름, 위치, 포함 변수, override 패턴, placeholder 형식 모두 일치 →
  **100%**
...
**5종 인프라 에러 패턴** (`SyntaxError: Unexpected token` /
`Invalid environment variables` /
`Request/TextEncoder/ReadableStream is not defined` /
`mockResolvedValue is not a function`) — 전체 unit suite grep **0건**.
...
> "PDCA Check에서 외부 차단·의도 cut만 남았으면 iterate 건너뛰고 분리 사이클로
> 이관"
...
→ **iterate 불필요** (코드 자체는 100% 완성). → **Phase 6는 별 세션 또는 단순
머지 워크플로**로 진행 권고.
...
Phase 6를 본 세션에서 진행 (commit + push + PR + CI 대기). 소요: ~10-15분 (+ CI
대기).
