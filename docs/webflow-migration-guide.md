...
- **전략**: Webflow 페이지 이관 시 Tailwind 토큰 우선 사용, 미매핑 색상은 CSS
  변수로 임시 보존
...
- **전략**: 스크롤 애니메이션은 `framer-motion`의 `useInView`로, 텍스트 분할은
  CSS `@keyframes`로 대체

### 이미지

- Webflow: 상대경로 `images/...` + CDN (`imagedelivery.net`)
- **전략**: 로컬 이미지 → `public/images/` 이동 후 `next/image` 적용, CDN
  이미지는 URL 유지
...
5. 인라인 스타일(`opacity:0; transform:...`) → Framer Motion `initial`/`animate`
   변환
