...
- **Memory-first triage**: heap OOM 가설(realtime/repository) 을 가장 먼저 확인
  — fix 가 1 옵션 변경(`maxWorkers=2`, `--max-old-space-size`) 이면 8건 중 4-5
  건 동시 해소 가능
...
- 수정한 file 외에 jest.config.js, jest.setup.js 변경분 grep 으로 영향 범위 확인
...
| ---------------- | ---------------------------------- | -------------------------- | --------------------------- |
| 1, 6, 3, 5, 2, 4 | realtime/**tests**/\* (4 file)     | Next.js 14 polyfill        | realtime-jest-polyfill-debt |
| 7, 8             | repositories/**tests**/\* (2 file) | DB mock 누락               | repository-test-mock-debt   |
...
- F1 패턴 시범 fix (event-emitter.test.ts, 24× afterEach + shutdown) — git
  history 보존, 별 사이클 활성화 시 즉시 사용
...
- E2E chromium suite (plan §2 E) — `route-auth-mock-debt` 와 연동 가능성, 별
  분석
- F1 패턴을 4 realtime file 에 일괄 적용 — `realtime-jest-polyfill-debt` 의
  mission
