...
- **Audit Log Viewer**:
  ...
  - `risk-indicator`: 리스크 점수에 따른 색상 변화 (0.0~0.3: `success`, 0.4~0.6: `warning`, 0.7+: `destructive`)
- **System Health Dashboard**:
  - `monitor-card`: CPU, 메모리, 레이턴시 지표를 담는 카드
  - `agent-status-grid`: 에이전트(`ArtistAgent` 등)의 활성 상태를 나타내는 미니 그리드
...
- **1.3.0-alpha** (2026-05-10) — **Enterprise Phase 확장**: 관리자 대시보드,
  감사 로그, 에이전트 모니터링 UI 패턴 추가 (§8.4, §8.5). 리스크 점수 기반
  시각 지표 정의.
- **1.2.1-alpha** (2026-04-12) — §2.1 재구성: **브랜드 의미론**과 **shadcn 슬롯
  ...
  mapping 변수 8개를 dead code 로 제거. WCAG 검증 쌍 재측정.
