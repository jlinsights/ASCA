...
import { Search, User, Filter, Award, Calendar, Star, MapPin, Mail, Globe } from "lucide-react"
                ...
                <Card key={artist.id} className="group overflow-hidden border-0 shadow-md hover:shadow-xl transition-all duration-300 dark:shadow-gray-900/20 dark:hover:shadow-gray-900/40">
                  <div className="relative aspect-square overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3">
                      <Badge 
                        variant="default"
                        className="bg-white/90 text-foreground border-white/30 dark:bg-gray-900/90 dark:text-foreground dark:border-gray-700/50"
                      >
                    ...
                    <div className="absolute top-3 right-3">
                      <Badge 
                        className={`${getArtistTypeBadgeColor(artist.artist_type)} border`}
                      >
                        {artist.artist_type}
                      </Badge>
                    </div>

                    {/* Title Badge - 직위가 있는 경우만 표시 */}
                    {artist.title && (
                ...
                <Card key={artist.id} className="group overflow-hidden border-0 shadow-md dark:shadow-gray-900/20">
                  <div className="relative aspect-square overflow-hidden">
                    ...
                    <div className="absolute top-2 left-2 flex flex-col gap-1">
                      <Badge 
                        variant="default"
                        className="text-xs px-2 py-1 bg-white/90 text-foreground border-white/30 dark:bg-gray-900/90 dark:text-foreground dark:border-gray-700/50"
                      >
                        {artist.membership_type}
                      </Badge>
                    </div>
                    ...
                    <div className="absolute top-2 right-2">
                      <Badge 
                        className={`text-xs px-2 py-1 ${getArtistTypeBadgeColor(artist.artist_type)} border`}
                      >
                        {artist.artist_type}
                      </Badge>
                    </div>
                    ...
                    {artist.title && (
                      <div className="absolute bottom-2 left-2">
                        <Badge 
                          variant="secondary"
                          className="text-xs px-2 py-1 bg-purple-100 text-purple-800 border-purple-200 dark:bg-purple-900/30 dark:text-purple-300 dark:border-purple-700"
                        >
                          {artist.title}
                        </Badge>
                      </div>
                  ...
                  <div className="flex items-end gap-4 px-6 -mt-8 relative">
                    ...
                    <div className="flex-1 pb-2">
                      ...
                      <p className="text-muted-foreground">
                        {selectedArtist.nameEn} • {selectedArtist.birthYear}년생
                      </p>
                      <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
                        ...
                        {selectedArtist.website && (
                          <a href={selectedArtist.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-celadon">
                  ...
                  <TabsContent value="about" className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground leading-relaxed">
                        {selectedArtist.bio[language as keyof typeof selectedArtist.bio]}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <ul className="space-y-1 text-muted-foreground">
                        {selectedArtist.education.map((edu, index) => (
                          <li key={index}>• {edu}</li>
                        ))}
                      </ul>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-4 text-center">
                        <div className="p-3 bg-muted rounded-lg">
                          <div className="text-2xl font-bold text-celadon">{selectedArtist.stats.artworksCount}</div>
                          <div className="text-sm text-muted-foreground">{language === 'ko' ? '작품' : 'Artworks'}</div>
                        </div>
                        <div className="p-3 bg-muted rounded-lg">
                          <div className="text-2xl font-bold text-celadon">{selectedArtist.stats.exhibitionsCount}</div>
                          <div className="text-sm text-muted-foreground">{language === 'ko' ? '전시' : 'Exhibitions'}</div>
                        </div>
                        <div className="p-3 bg-muted rounded-lg">
                          <div className="text-2xl font-bold text-celadon">{selectedArtist.stats.viewCount}</div>
                          <div className="text-sm text-muted-foreground">{language === 'ko' ? '조회수' : 'Views'}</div>
                        </div>
                        <div className="p-3 bg-muted rounded-lg">
                          <div className="text-2xl font-bold text-celadon">{selectedArtist.stats.followers}</div>
                        ...
                        <Card key={index}>
                          <CardContent className="p-4">
                            <div className="flex items-start justify-between">
                              <div>
                                <h4 className="font-medium mb-1">{exhibition.title}</h4>
                                <p className="text-sm text-muted-foreground mb-1">{exhibition.venue}</p>
