...
const ImageViewer = lazy(() =>
  import('@/components/image-viewer').then(mod => ({ default: mod.ImageViewer }))
)
const ZoomableImageViewer = lazy(() =>
  import('@/components/dynamic-imports').then(mod => ({ default: mod.default.ZoomableImageViewer }))
)
const AdvancedGallerySearch = lazy(() =>
  import('@/components/dynamic-imports').then(mod => ({
    default: mod.default.AdvancedGallerySearch,
  }))
)
    ...
    const filtered = artworks.filter(artwork => {
      ...
      if (searchTerm) {
        ...
        matchesSearch =
          ...
          (artwork.materials?.some((material: string) =>
            ...
            return (
              <Card key={artwork.id} className='group hover:shadow-lg transition-all duration-300'>
                <CardContent className='p-0'>
                  {/* 이미지 */}
                  <div
                    className={`relative ${getImageAspectRatio(artwork)} overflow-hidden rounded-t-lg`}
                  >
                    <ArtworkImage
                      src={artwork.image_url || '/images/artworks/calligraphy-scene-1.jpg'}
                      alt={artwork.title}
                  ...
                  <div className='p-4 space-y-3'>
                    <div>
                      <h3 className='font-semibold text-sm line-clamp-2 mb-1'>{artwork.title}</h3>
