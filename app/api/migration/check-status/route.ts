  ...
  try {
    ...
    return NextResponse.json({
      ...
      notes: {
        ...
        artworks:
          artworks.length > 0
            ? `${artworks.length}개 레코드 확인됨`
            : '테이블이 비어있음 (작품 데이터 없음)',
        exhibitions:
          exhibitions.length > 0
            ? `${exhibitions.length}개 레코드 확인됨`
            : '테이블이 비어있음 (전시회 데이터 없음)',
        events:
          events.length > 0
            ? `${events.length}개 레코드 확인됨`
            : '테이블이 비어있음 (이벤트 데이터 없음)',
        notices:
          notices.length > 0
            ? `${notices.length}개 레코드 확인됨`
            : '테이블이 비어있음 (공지사항 데이터 없음)',
      },
    })
  } catch (error) {
    return NextResponse.json(
