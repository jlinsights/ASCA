    ...
    const result = await withPerformanceLog(
      ...
      async () => {
        ...
        if (params.query) {
          query = query.where(
            or(
              like(members.first_name_ko, `%${params.query}%`),
              like(members.last_name_ko, `%${params.query}%`),
        ...
        // Apply status filter
        if (params.status) {
          query = query.where(eq(members.membership_status, params.status)) as typeof query;
        }

        // Apply level filter
        if (params.level) {
          query = query.where(eq(members.membership_level_id, params.level)) as typeof query;
        }
...
export async function searchMembers(params: MemberSearchParams) {
  const result = await withPerformanceLog('members.advancedSearch', async () => {
    ...
    if (params.status) {
      conditions.push(eq(members.membership_status, params.status));
    }

    if (params.level) {
      conditions.push(eq(members.membership_level_id, params.level));
    }
