...
export function ExhibitionManagementClient() {
  ...
  useEffect(() => {
    ...
    const loadExhibitions = async () => {
      ...
      const { data, error } = await fetchArtistExhibitions(clerkUser.id, {
    ...
    loadExhibitions()
  }, [isLoaded, isSignedIn, clerkUser, router, statusFilter])
      ...
      <div className='container mx-auto px-4 py-8'>
        <div className='max-w-6xl mx-auto'>
          ...
          <div className='flex items-center justify-between mb-8'>
            <div>
              <h1 className='text-3xl font-serif font-bold text-foreground mb-2'>전시 관리</h1>
              <p className='text-muted-foreground'>내 전시를 관리하고 새로운 전시를 만들어보세요</p>
            </div>
            <Link href='/exhibitions/create'>
              <Button className='bg-celadon-green hover:bg-celadon-green/90'>
                <Plus className='w-4 h-4 mr-2' />새 전시 만들기
              </Button>
            </Link>
          </div>
          ...
          <div className='grid grid-cols-2 md:grid-cols-5 gap-4 mb-8'>
            <Card>
              <CardContent className='p-4 text-center'>
                <div className='text-3xl font-bold text-celadon-green mb-1'>{stats.total}</div>
            ...
            <Card>
              <CardContent className='p-4 text-center'>
                <div className='text-3xl font-bold text-emerald-600 mb-1'>{stats.current}</div>
            ...
            <Card>
              <CardContent className='p-4 text-center'>
                <div className='text-3xl font-bold text-slate-600 mb-1'>{stats.past}</div>
            ...
            <Card>
              <CardContent className='p-4 text-center'>
                <div className='text-3xl font-bold text-temple-gold mb-1'>{stats.featured}</div>
            ...
            <Card>
              <CardContent className='p-12 text-center'>
                <div className='max-w-md mx-auto'>
                  ...
                  <Link href='/exhibitions/create'>
                    <Button className='bg-celadon-green hover:bg-celadon-green/90'>
                      <Plus className='w-4 h-4 mr-2' />첫 전시 만들기
                    </Button>
                  </Link>
                </div>
              </CardContent>
                              ...
                              <Badge className={getStatusColor(exhibition.status)}>
                                {
                                  EXHIBITION_STATUS_LABELS[
                                    exhibition.status as keyof typeof EXHIBITION_STATUS_LABELS
                                  ]?.ko
                                }
                              </Badge>
