...
export default function ContestDetailPage() {
  ...
  useEffect(() => {
    if (!isLoaded) return
    setIsAuthenticated(!!(isSignedIn && clerkUser))
              ...
              <CardHeader className='pb-4'>
                <div className='flex items-start justify-between mb-4'>
                  <div className='flex items-center gap-3 flex-wrap'>
                    ...
                    <Badge variant='outline'>{CONTEST_CATEGORY_LABELS[contest.category]?.ko}</Badge>
                    <Badge variant='outline'>{CONTEST_TYPE_LABELS[contest.contestType]?.ko}</Badge>
                  </div>
                ...
                <CardContent>
                  <div className='space-y-3'>
                    {contest.prizes.map((prize, index) => (
                      <div
                        key={index}
                        className='flex items-start justify-between p-4 rounded-lg border border-border/50'
                      >
                        <div>
                          <h4 className='font-semibold text-foreground'>{prize.rank}</h4>
                          <p className='text-sm text-muted-foreground'>{prize.count}명 선정</p>
                        </div>
            ...
            <Card className='border-celadon-green/30'>
              ...
              <CardContent className='space-y-4'>
                ...
                {deadline.isExpired && (
                  <Alert>
                    <AlertCircle className='h-4 w-4' />
                    <AlertDescription>접수가 마감되었습니다</AlertDescription>
                  </Alert>
                )}
            ...
            <Card>
              ...
              <CardContent className='space-y-3 text-sm'>
                ...
                <div>
                  ...
                  <p className='font-medium text-foreground'>
                    {contest.viewCount.toLocaleString()}회
                  </p>
                </div>
              </CardContent>
            </Card>
